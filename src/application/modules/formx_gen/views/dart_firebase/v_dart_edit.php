          ...
          foreach ($non_pk as $row) {
            if ($row->type == 'latitude' || $row->type == 'longitude') { continue;
              ...
              new Container(
                ...
                child: _image".$row->column_name." == null
                    ? Center(child:Text('No image selected.'))
                    : Image.file(_image".$row->column_name."),
              ),
              ";
            }elseif($row->type == 'date'){
              $string .= "
                  ...
                  onPressed: () async {
                    if (_key.currentState.validate()) {
                      try {
                        ...
                        await ".$class_name."Service().edit(widget.item.id,
                          ".$class_name."(";
                          foreach ($non_pk as $row) {
                            if ($row->type == 'img') 
                              $string.="
                            ...
                            elseif ($row->type == 'int') 
                              $string.="
                            ".$row->column_name.": int.parse(_".$row->column_name."Controller.text),";
