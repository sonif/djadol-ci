...
class Api extends REST_Controller
{
    ...
    public function all_post($form_id = null)
    {
        ...
        $form_param = $this->Formx_model->get_param_rest($form_id);
        $ar_haklistakses = get_listakses_formx(
            $this->usergroup_id,
            "form_" . $form_id,
        );
        ...
        $iDisplayLength = $this->input->get_post("length")
            ? $this->input->get_post("length")
            : 30;
        $iDisplayStart = $this->input->get_post("page")
            ? $this->input->get_post("page") * $iDisplayLength
            : 0;

        $this->Formx_model->set_table($form_id, "view");
        $t = $this->Formx_model->get_limit_data(
            $iDisplayStart,
            $iDisplayLength,
        );
            ...
            foreach ($get_data as $d) {
                ...
                foreach ($form_param->result() as $p) {
                    ...
                    if ($p->type == "select") {
                        ...
                        if (!empty($d->{$p->column_name})) {
                            $row[$p->column_name] = number_format(
                                $d->{$p->column_name},
                                0,
                                ",",
                                ".",
                        ...
                        if (!empty($d->{$p->column_name})) {
                            $row[$p->column_name] = base_url(
                                $p->path_upload . "/" . $d->{$p->column_name},
                            );
                        } else {
                            $row[$p->column_name] = "";
                        }
    ...
    public function form_action_post()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($form_param->result() as $p) {
                ...
                if (
                    ...
                    if (!empty($this->input->post($p->column_name))) {
                        ...
                        $data[$p->column_name] = password_hash(
                            $this->input->post($p->column_name),
                            PASSWORD_BCRYPT,
                        );
                    }
            ...
            if ($this->input->post("parent_id")) {
                $data[$this->input->post("parent_column")] = $this->input->post(
                    "parent_id",
                );
            }
            ...
            if (empty($id)) {
                ...
                // }
                if (
                    method_exists($this->M_before_insert, $m_form->form_table)
                ) {
                    $data = $this->M_before_insert->{$m_form->form_table}(
                        $data,
                    );
                }
                # code...
                ...
                if ($id = $this->Formx_model->insert($data)) {
                    ...
                    if (
                        ...
                        $this->M_after_insert->{$m_form->form_table}(
                            $id,
                            $data,
                        );
                    }
                ...
                if ($row) {
                    ...
                    if (
                        method_exists(
                            $this->M_before_update,
                            $m_form->form_table,
                        )
                    ) {
                        $data = $this->M_before_update->{$m_form->form_table}(
                            $id,
                            $data,
                        );
                    }
    ...
    public function _upload($targetPath = "", $form_upload_name = "doc_file")
    {
        ...
        $response = $this->m_public_function->upload(
            $config,
            $form_upload_name,
        );
